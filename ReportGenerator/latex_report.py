...
class LatexReport:
    ...
    def __replace_map_dict(self) -> dict:
        if not self.report_content:
            raise ValueError("Report content should have been set.")
        return {
    ...
    def convert_to_pdf(report_path: str):
        ...
        try:
            pycall = "pdflatex {}/{}.tex".format(
                os.path.join(current_wc, report_dir), LatexReport.__latex_report_name
            )
    ...
    def generate_latex_report(self, target_dir: str):
        ...
        """
        latex_report_path = self.__get_latex_report_file_path(target_dir=target_dir)
        ...
        replaced_report = self.__replace_latex_template_content(
    ...
    def __get_latex_report_file_path(self, target_dir: str) -> str:
        ...
        template_dir = ReportHelper._get_template_folder(self.__latex_template_dir)
    ...
    def __replace_latex_template_content(self, report_template: str) -> str:
    ...
    def __generate_python_authors(self) -> str:
        ...
        """
        author_template = "\\author{author_pos}{{{author_name}}}\n"
        authors_content = ""
        for pos, author in enumerate(self.report_content.authors_list):
            authors_content += author_template.format(
                author_pos="i" * (pos + 1), author_name=author
            )
    ...
    def __generate_python_chapters(self):
        ...
        for case in sorted(cases_dict):
            ...
            chapters_content += chapter_template.format(
                temp_name_key=case_name, temp_key_key=case_key
            )

            # Input section description
            case_description = self.report_content.case_description_dict.get(
                case_key, "No case description\n"
            )

            chapters_content += case_description_template.format(
                case_description=case_description
            )

            # Input figure sections
            for section in case_figs:
                section_content = self.__generate_python_section(
                    sectionname=section, case_figures=case_figs[section]
                )
        ...
        if not chapters_content:
            error_mssg = (
                "" + "No LaTeX content was generated from " + "{}.".format(cases_dict)
            )
            raise ValueError(error_mssg)
