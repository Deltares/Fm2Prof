...
class Test_Fm2ProfRunner:
    ...
    def test_given_inifile_then_no_exception_is_risen(self):
        ...
        assert os.path.exists(ini_file_path), '' \
            'Test File {} was not found'.format(ini_file_path)
...
class Test_generate_cross_section:
    ...
    def test_when_no_input_param_dict_is_given_then_expected_exception_risen(
        ...
        # 2. Set expectations
        expected_error = 'No input parameters (from ini file)' + \
    ...
    @pytest.mark.unittest
    def test_when_no_fm_model_data_is_given_then_expected_exception_risen(
            self):
        # 1. Set up test data
        runner = Fm2ProfRunner(None)
        ...
        # 2. Set expectations
        expected_error = 'No FM data given for new cross section' + \
            ' {}'.format(test_css_name)
    ...
    def test_when_all_parameters_are_correct_then_returns_expected_css(self):
        ...
        try:
            ...
            pytest.fail(
                'No expected exception but was risen:' +
                ' {}'.format(str(e_info)))
...
class Test_set_fm_data_to_cross_section:
    ...
    def test_when_no_cross_section_given_then_no_exception_risen(self):
        ...
        try:
            ...
            pytest.fail(
                'No expected exception but was thrown: {}'.format(str(e_info)))
