...
class CompareHelper:
    ...
    def convert_ZW_to_symmetric_css(gmlist):
        ...
        return [
            np.append(list(reversed(gmlist[0])), gmlist[0]),
    ...
    @staticmethod
    def _friction_manning_from_deepest_point(
        css, manning_coefficient: float = 0.03
    ) -> Tuple:
...
class CompareIdealizedModel:
        ...
        for cs in range(len(Y)):
            ...
            if plt_s_flag:
                if case_name == "case_05_dyke":
                    ...
                    self.__plot_cs(
                        fm2prof_fig_dir, tx, tz, plt_x, plt_z, y, sumError, plt_s, ts
                    )
            else:
                if case_name == "case_05_dyke":
                    self.__plot_cs(
    ...
    def _compare_roughness(
        ...
        for cs in range(len(Y)):
            ...
            self.__plot_roughness(
                fm2prof_fig_dir, tH_pos, tR_pos, hpos, rpos, y, MainChannel
            )
    ...
    def __plot_volume(self, df, cs, output_folder):
        ...
        plt.savefig(
            os.path.join(output_folder, "{}_volumegraph.png".format(cs_no_period))
        )
    ...
    def __get_geometry_data(self, input_file: str):
        ...
        TA = []
        n = 0
        assert os.path.exists(input_file), "" + "Input file {} does not exist".format(
            input_file
        )
        ...
        for i in range(1, len(plt_z) - 1):
            ...
            for j in range(n, len(tz)):
                ...
                if abs(est_x[i] - float(tx[j])) < 1e-5:
                    ...
                    and abs(est_x[i] - est_x[i - 1]) < 1e-5
                ):
                    est_z.append(tz[-1])
                    flag1 = 1
                    break
    ...
    def __plot_cs(
        ...
        if plt_s is not None and plt_s:
            axh.plot(
                plt_s, plt_z, label="FM2PROF flow width", linestyle=":", color="#ff7f0e"
            )
    ...
    def __convert_zw2xz(
        self, z: list, w: list, tx: list, max_width: float, flow_width=None
    ):
    ...
    def __plot_roughness(self, fig_dir, tH_pos, tR_pos, H_pos, R_pos, y, MainChannel):
        ...
        if MainChannel:
            titlestr = (
                "Water level dependent main channel roughness "
                + "at chainage "
                + str(y)
            )
        else:
            titlestr = (
                "Water level dependent floodplain1 roughness " + "at chainage " + str(y)
            )
        axh.set_title(titlestr)
        plt.grid()
        plt.tight_layout()
                ...
                if MainChannel:
                    ...
                    if case_name == "case_06_plassen":
                        if y >= 1420 and y <= 1580:
                            if d < 2.0:
                                Cz = ((-0.53 + 10) * 30 / ((-0.53 + 10) * 2 + 30)) ** (
                                    1 / 6
                                ) / 0.07
                            elif (
                                d >= 2.0 and d < 2.0 + 0.01
                            ):  # +0.00048m in reality (filling lake completely)
                                Cz = ((30 * (b + 8)) / (2 * (b + 8) + 30)) ** (
                                    1 / 6
                                ) / 0.07
                            else:
                                Cz = (100 * (d - 2.0) / (2.0 * (d + 48))) ** (
                                    1 / 6
                                ) / 0.07
                        else:
                            if d > 2.0 + 0.01:
                    ...
                    if d > 2.0:
                        if case_name == "case_02_compound":
                            ...
                            if d > 3.0 and d <= 3.0 + th:
                                Cz = (
                                    (100 * (d - 3.0) + 100 / th * (d - 3.0))
                                    / (4 * (1.0 / th) * (d - 3.0) + 2 * (d - 3.0) + 100)
                                ) ** (1 / 6) / 0.07
                            elif d > 3.0 + th:
                                Cz = (
                                    (100 * (d - 3.0) + 100)
