...
class Test_generate_cross_section_instance:
    ...
    def test_when_correct_input_dict_is_given_CrossSection_initialises(self):
        ...
        try:
            ...
            pytest.fail(f"No expected exception but was risen:" + " {e_info}")
...
class Test_cross_section_construction:
    def test_build_geometry(self):
        ...
        assert all(
            [
                abs(css_total_volume[i] - css._css_total_volume[i]) < tol
                for i in range(len(css_total_volume))
            ]
        )
    ...
    def test_calculate_correction(self):
        ...
        # 4. Verify final expectations TODO: fix test
